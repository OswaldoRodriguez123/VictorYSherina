class Presupuesto {
  ...
  validarClases = (cantidadClases) => {
    if (
      isNaN(cantidadClases) ||
      cantidadClases <= 0 ||
      !Number.isInteger(cantidadClases)
    ) {
      return false;
    }
    return true;
  ...
  generarClases = (cantidadClases) => {
    ...
    for (let i = 1; i <= cantidadClases; i++) {
      const objClase = {
        fechaClase: new Date(Date.now() + i * 24 * 60 * 60 * 1000)
          .toISOString()
          .slice(0, 10),
      };
  ...
  calcularTotal = () =>
    (this.totalPresupuesto = this.clases.length * this.costoClase);
  primeraClase = () =>
    this.clases
  ...
  enviarCorreo = () =>
    alert(
      "El presupuesto fue enviado exitosamente a su correo, gracias por confiar en nosotros"
    );
}
